<div class="container">
    ...
    <table class="table table-striped table-hover">
        ...
        @foreach($product_property_groups as $property_group)
            <tr>
                    ...
                    <ul class="list-group">
                        @foreach($property_group->properties()->where('product_id',$product->id)->get() as $property)
                            <div class="row flex justify-content-center align-item-center">
                                ...
                                <i style="cursor: pointer;" class="ti-trash m-r-5 col-2" wire:click="$dispatch('deleteSellerProductProperty',{'property_group_id':{{$property_group->id}},
                ...
                <td class="text-center align-middle">
                    <a class="btn btn-outline-danger" wire:click="$dispatch('deleteSellerProductPropertyGroup',{'property_group_id':{{$property_group->id}}})">
                        حذف
                    </a>
                </td>
            </tr>
    ...
    @if($product->properties()->exists() && $product->propertyGroups()->exists())
        <div class="text-center mt-4">
            <a href="{{ route('seller.product.file.list', $product->id) }}"
...
@section('scripts')
    <script>
        Livewire.on('deleteSellerProductPropertyGroup', (event) => {
            Swal.fire({
    ...
    <script>
        Livewire.on('deleteSellerProductProperty', (event) => {
            Swal.fire({
                title: "آیا از حذف مطمئن هستید؟",
                icon: "warning",
                showCancelButton: true,
