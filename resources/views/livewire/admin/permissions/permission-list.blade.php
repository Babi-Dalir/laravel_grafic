<div class="table overflow-auto" tabindex="8">
    <div class="form-group row">
        <label class="col-sm-2 col-form-label">جستجو (عنوان مجوز)</label>
    ...
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <th class="text-center align-middle text-primary">عنوان مجوز</th>
        ...
        @forelse($permissions as $index=>$permission)
            <tr>
                ...
                <td class="text-center align-middle">
                    <a class="btn btn-outline-danger" wire:click="$dispatch('deletePermission',{'id':{{$permission->id}}})">
                        حذف
                    </a>
                </td>
                <td class="text-center align-middle">{{\Hekmatinasser\Verta\Verta::instance($permission->created_at)->format('%d%B، %Y')}}</td>
            </tr>
        @empty
            <tr>
                <td colspan="9" class="text-center py-5" style="background-color: #f9f9f966;">
                    <div class="empty-state">
                        ...
                        <svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="mb-3">
                            ...
                            <p class="text-muted">مجوزی با عبارت <strong class="text-danger">"{{ $search }}"</strong> در سیستم ثبت نشده است.</p>
        ...
        Livewire.on('deletePermission', (event) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.dispatch('destroy_permission',{id : event.id})
                    Swal.fire({
