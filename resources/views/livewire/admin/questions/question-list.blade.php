<div class="table overflow-auto" tabindex="8">
    ...
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <th class="text-center align-middle text-primary">متن پرسش یا پاسخ</th>
        ...
        @foreach($questions as $index=>$question)
            <tr>
                ...
                <td class="text-center align-middle">{{$question->product->name}}</td>
                ...
                <td class="text-center align-middle">
                    @if($question->status === \App\Enums\QuestionStatus::Approved->value)
                        <span class="cursor-pointer badge badge-success">تایید شده</span>
                    @elseif($question->status === \App\Enums\QuestionStatus::Draft->value)
                        <span class="cursor-pointer badge badge-warning">در انتظار تایید</span>
                    @elseif($question->status === \App\Enums\QuestionStatus::Rejected->value)
                        <span class="cursor-pointer badge badge-danger">تایید نشده</span>
                    @endif
                </td>
                <td class="text-center align-middle">{{\Hekmatinasser\Verta\Verta::instance($question->created_at)->format('%d%B، %Y')}}</td>
            </tr>
        @endforeach
    </table>
