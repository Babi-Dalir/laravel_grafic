    ...
    <table class="table table-striped table-hover">
        ...
        @foreach($banners as $index=>$banner)
            <tr>
                ...
                <td class="text-center align-middle">
                    <a class="btn btn-outline-danger" wire:click="$dispatch('deleteBanner',{'id':{{$banner->id}}})">
                        حذف
                    </a>
                </td>
                <td class="text-center align-middle">{{\Hekmatinasser\Verta\Verta::instance($banner->created_at)->format('%d%B، %Y')}}</td>
            </tr>
        @endforeach
    </table>
...
@section('scripts')
    <script>
        Livewire.on('deleteBanner', (event) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.dispatch('destroy_banner',{id : event.id})
                    Swal.fire({
