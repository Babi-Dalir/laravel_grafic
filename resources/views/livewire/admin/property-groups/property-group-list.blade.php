<div class="table overflow-auto" tabindex="8">
    <div class="form-group row">
        <label class="col-sm-2 col-form-label">جستجو (نام گروه ویژگی)</label>
    ...
    <table class="table table-striped table-hover">
        ...
        <tbody>
        @forelse($property_groups as $index=>$property_group)
            <tr>
                ...
                {{-- 🟢 اصلاح: حذف کد تدافعی بی‌مورد به دلیل وجود ضمانت cascadeOnDelete در دیتابیس --}}
                <td class="text-center align-middle">{{$property_group->category->name}}</td>
                ...
                <td class="text-center align-middle">
                    <a class="btn btn-outline-danger"
                       wire:click="$dispatch('deletePropertyGroup',{'id':{{$property_group->id}}})">
                        حذف
                    </a>
                </td>
                <td class="text-center align-middle">{{\Hekmatinasser\Verta\Verta::instance($property_group->created_at)->format('%d %B، %Y')}}</td>
            </tr>
        @empty
            <tr>
                <td colspan="6" class="text-center py-5" style="background-color: #f9f9f966;">
                    <div class="empty-state">
                        ...
                        <p class="text-muted">گروه ویژگی با عبارت <strong class="text-danger">"{{ $search }}"</strong> در سیستم ثبت نشده است.</p>

                        {{-- 🟢 اصلاح: تصحیح درخت DOM و بستن تگ‌ها در جایگاه درست خودشان --}}
                        @if($search)
...
@section('scripts')
    <script>
        Livewire.on('deletePropertyGroup', (event) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.dispatch('destroy_property_group', {id: event.id})
                    Swal.fire({
