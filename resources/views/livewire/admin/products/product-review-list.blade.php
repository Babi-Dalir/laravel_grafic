<div class="table overflow-auto" tabindex="8">
    ...
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <th class="text-center align-middle text-primary">عنوان نقد وبررسی</th>
        ...
        @foreach($reviews as $index=>$review)
            <tr>
                ...
                <td class="text-center align-middle">
                    <a class="btn btn-outline-danger" wire:click="$dispatch('deleteReview',{'id':{{$review->id}}})">
                        حذف
                    </a>
                </td>
                <td class="text-center align-middle">{{\Hekmatinasser\Verta\Verta::instance($review->created_at)->format('%d%B، %Y')}}</td>
            </tr>
        @endforeach
    </table>
...
@section('scripts')
    <script>
        Livewire.on('deleteReview', (event) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.dispatch('destroy_review',{id : event.id})
                    Swal.fire({
