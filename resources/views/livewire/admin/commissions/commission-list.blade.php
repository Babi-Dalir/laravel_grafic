    ...
    <table class="table table-striped table-hover">
        ...
        @foreach($commissions as $index=>$commission)
            <tr>
                ...
                <td class="text-center align-middle">{{$commission->commission_percent}}</td>
                <td class="text-center align-middle">{{$commission->category->name}}</td>
                <td class="text-center align-middle">
                    <a class="btn btn-outline-info" href="{{route('commissions.edit',$commission->id)}}">
                        ویرایش
                    </a>
                </td>
                <td class="text-center align-middle">{{\Hekmatinasser\Verta\Verta::instance($commission->created_at)->format('%d%B، %Y')}}</td>
            </tr>
        @endforeach
    </table>
