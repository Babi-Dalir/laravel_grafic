    ...
    <table class="table table-striped table-hover">
        ...
        @foreach($sliders as $index=>$slider)
            <tr>
                ...
                <td class="text-center align-middle">
                    <a class="btn btn-outline-danger" wire:click="$dispatch('deleteSlider',{'id':{{$slider->id}}})">
                        حذف
                    </a>
                </td>
                <td class="text-center align-middle">{{\Hekmatinasser\Verta\Verta::instance($slider->created_at)->format('%d%B، %Y')}}</td>
            </tr>
        @endforeach
    </table>
...
@section('scripts')
    <script>
        Livewire.on('deleteSlider', (event) => {
            Swal.fire({
