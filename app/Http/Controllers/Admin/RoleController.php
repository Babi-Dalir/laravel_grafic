...
class RoleController extends Controller
{
     ...
     */
    public function store(RolerRequest $request)
    {
        Role::query()->create([
            'name'=>$request->input('name')
        ]);
        return redirect()->route('roles.index')->with('message', 'نقش جدید با موفقیت ایجاد شد');
    }
     ...
     */
    public function update(RolerRequest $request, string $id)
    {
        $role = Role::query()->find($id);
        $role->update([
            'name'=>$request->input('name')
